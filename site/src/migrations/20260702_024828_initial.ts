...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  `)
  await db.run(sql`CREATE INDEX \`projects_parent_idx\` ON \`projects\` (\`parent_id\`);`)
  ...
  await db.run(sql`CREATE INDEX \`repos_project_idx\` ON \`repos\` (\`project_id\`);`)
  ...
  await db.run(sql`CREATE TABLE \`activity_events\` (
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`title\` text NOT NULL,
  	\`kind\` text DEFAULT 'commit',
  	\`repo_full_name\` text,
  	\`url\` text,
  ...
  `)
  await db.run(sql`CREATE UNIQUE INDEX \`activity_events_external_id_idx\` ON \`activity_events\` (\`external_id\`);`)
  await db.run(sql`CREATE INDEX \`activity_events_project_idx\` ON \`activity_events\` (\`project_id\`);`)
  await db.run(sql`CREATE INDEX \`activity_events_updated_at_idx\` ON \`activity_events\` (\`updated_at\`);`)
  await db.run(sql`CREATE INDEX \`activity_events_created_at_idx\` ON \`activity_events\` (\`created_at\`);`)
  await db.run(sql`CREATE TABLE \`knowledge_items\` (
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`title\` text NOT NULL,
  	...
  	\`updated_at\` text DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ', 'now')) NOT NULL,
  	\`created_at\` text DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ', 'now')) NOT NULL,
  	FOREIGN KEY (\`artifact_id\`) REFERENCES \`media\`(\`id\`) ON UPDATE no action ON DELETE set null,
  	FOREIGN KEY (\`project_id\`) REFERENCES \`projects\`(\`id\`) ON UPDATE no action ON DELETE set null
  );
  `)
  await db.run(sql`CREATE INDEX \`knowledge_items_artifact_idx\` ON \`knowledge_items\` (\`artifact_id\`);`)
  await db.run(sql`CREATE INDEX \`knowledge_items_project_idx\` ON \`knowledge_items\` (\`project_id\`);`)
  await db.run(sql`CREATE INDEX \`knowledge_items_updated_at_idx\` ON \`knowledge_items\` (\`updated_at\`);`)
  await db.run(sql`CREATE INDEX \`knowledge_items_created_at_idx\` ON \`knowledge_items\` (\`created_at\`);`)
  await db.run(sql`CREATE TABLE \`tasks_artifacts\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`id\` text PRIMARY KEY NOT NULL,
  	\`label\` text,
  ...
  `)
  await db.run(sql`CREATE INDEX \`tasks_artifacts_order_idx\` ON \`tasks_artifacts\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`tasks_artifacts_parent_id_idx\` ON \`tasks_artifacts\` (\`_parent_id\`);`)
  await db.run(sql`CREATE TABLE \`tasks\` (
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`title\` text NOT NULL,
  	\`body\` text,
  	\`status\` text DEFAULT 'Idea',
  	\`list\` text DEFAULT 'active',
  	\`scope\` text DEFAULT 'Sarapis',
  	\`project_id\` integer,
  	\`priority\` numeric,
  	\`starred\` integer,
  	\`source\` text,
  	\`created_by\` text,
  	\`publish_to_activity\` integer DEFAULT false,
  	\`pinned\` integer DEFAULT false,
  	\`updated_at\` text DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ', 'now')) NOT NULL,
  	\`created_at\` text DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ', 'now')) NOT NULL,
  	FOREIGN KEY (\`project_id\`) REFERENCES \`projects\`(\`id\`) ON UPDATE no action ON DELETE set null
  ...
  await db.run(sql`CREATE TABLE \`payload_locked_documents_rels\` (
  	...
  	FOREIGN KEY (\`activity_events_id\`) REFERENCES \`activity_events\`(\`id\`) ON UPDATE no action ON DELETE cascade,
  	FOREIGN KEY (\`knowledge_items_id\`) REFERENCES \`knowledge_items\`(\`id\`) ON UPDATE no action ON DELETE cascade,
  ...
  await db.run(sql`CREATE INDEX \`payload_locked_documents_rels_activity_events_id_idx\` ON \`payload_locked_documents_rels\` (\`activity_events_id\`);`)
  await db.run(sql`CREATE INDEX \`payload_locked_documents_rels_knowledge_items_id_idx\` ON \`payload_locked_documents_rels\` (\`knowledge_items_id\`);`)
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  await db.run(sql`DROP TABLE \`knowledge_items\`;`)
  await db.run(sql`DROP TABLE \`tasks_artifacts\`;`)
